...
import com.example.simple_calendar.activities.EventActivity
import com.example.simple_calendar.activities.EventTypePickerActivity
import com.example.simple_calendar.activities.SnoozeReminderActivity
import com.example.simple_calendar.activities.TaskActivity
...
import com.example.simple_calendar.interfaces.TasksDao
import com.example.simple_calendar.interfaces.WidgetsDao
import com.example.simple_calendar.models.Event
import com.example.simple_calendar.receivers.AutomaticBackupReceiver
...
import com.example.simple_calendar.models.ListEvent
import com.example.simple_calendar.models.Task
import com.example.simple_calendar.services.MarkCompletedService
import com.example.simple_calendar.services.SnoozeService
...
fun Context.backupEventsAndTasks() {
    ...
    ensureBackgroundThread {
        ...
        val events = eventsHelper.getEventsToExport(
            eventTypes = config.autoBackupEventTypes.map { it.toLong() } as java.util.ArrayList<Long>,
    ...
    YEAR -> getString(R.string.yearly)
        ...
        when {
            seconds % YEAR == 0 -> resources.getQuantityString(
                com.simplemobiletools.commons.R.plurals.years,
                seconds / YEAR,
                seconds / YEAR
            )

            seconds % MONTH == 0 -> resources.getQuantityString(
                com.simplemobiletools.commons.R.plurals.months,
                seconds / MONTH,
                seconds / MONTH
            )

            seconds % WEEK == 0 -> resources.getQuantityString(
                com.simplemobiletools.commons.R.plurals.weeks,
                seconds / WEEK,
                seconds / WEEK
            )

            else -> resources.getQuantityString(
                com.simplemobiletools.commons.R.plurals.days,
                seconds / DAY,
                seconds / DAY
            )
        }
...
fun Context.getShortDaysFromBitmask(bitMask: Int): String {
    ...
    val weekDays = withFirstDayOfWeekToFront(
        resources.getStringArray(com.simplemobiletools.commons.R.array.week_days_short).toList()
    )
...
fun <T> Context.withFirstDayOfWeekToFront(weekItems: Collection<T>): java.util.ArrayList<T> {
    val firstDayOfWeek = config.firstDayOfWeek
    if (firstDayOfWeek == DateTimeConstants.MONDAY) {
        return weekItems.toMutableList() as java.util.ArrayList<T>
    }
    ...
    return rotatedWeekItems as java.util.ArrayList<T>
}
